...
### Decision: df003 이름은 `-(으)ㄴ 적이 있/없다`로 기록한다.

Reason: 기존 df003 정규식 gold에는 `있다` 계열뿐 아니라 `없는`, `없었다`처럼 경험 부재를 나타내는 예문도 포함되어 있다. 따라서 pilot item 이름과 config를 `경험 유무 서술`에 맞게 기록한다.
