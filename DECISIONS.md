...
Status: 2026-05-04 기준으로 이 결정은 “gold 원본은 Excel”이라는 원칙으로 확장되었다. 반복 자동화에서는 `gold_ps_??.xlsx` 같은 item/polyset-specific skeleton Excel도 사람 관리 원본으로 허용하고, 장기 aggregate `gold.xlsx`는 이후 통합 관리 파일로 둘 수 있다.
...
Status: 2026-05-02 이후에는 뉴스/일상 5,000행 고정 비교가 아니라 공통 prepared corpus batch를 사용한다. batch_002부터 비율은 일상대화 5,000행, 뉴스 700행, 비출판물 2,000행, 학습자 말뭉치 2,500행이다. 2026-05-04 이후 규칙 수정 여부는 사람이 확정한 labeled review의 FP/TP와 processed batch 정책을 기준으로 판단한다.
...
Status: 현재 브릿지는 별도 regex 버전보다 `rule_components.bridge_id`와 공용 bridge registry를 우선한다. ps_ce002의 `nde` bridge도 같은 방식으로 연결한다.
...
Status: 2026-05-04 기준으로 이 결정은 “dict 원본은 Excel”이라는 원칙으로 확장되었다. 통합 `dict.xlsx`뿐 아니라 `dict_ps_??.xlsx` 같은 item/polyset-specific skeleton/working dict도 Excel SSOT 계열로 인정한다. bundle은 항상 dict Excel에서 재생성하며 직접 수정하지 않는다.
...
Status: 2026-05-05 기준으로 이 결정은 모든 문법항목에 `ps_id` task unit을 부여하는 결정으로 확장되었다. `ps_id`는 더 이상 실제 group=c polyset만 뜻하지 않고, 단일 item wrapper와 multi-member polyset을 모두 포함하는 detect/encoder task id이다.

### Decision: 모든 문법항목은 `e_id`와 `ps_id`를 분리한 2-ID 체계로 운영한다.

Reason: a/b/c group에 따라 어떤 문법항목은 `e_id`, 어떤 문법항목은 `ps_id`로 detect하면 dict, rule_components, detect_rules, encoder export 코드가 계속 분기된다. 300개 자동화를 안정화하려면 모든 문법항목에 같은 task-unit layer를 두는 편이 단순하다. 따라서 `e_id`는 teaching_item_id로 유지하고, `ps_id`는 detect_unit_id이자 encoder_task_id로 사용한다. `ps_df003`처럼 member가 하나뿐인 task unit도 허용하며, 이 경우 runtime에서는 일반 item unit처럼 실행한다. `ps_ce002`처럼 member가 여러 개인 경우는 multi-member polyset unit으로 보고 명시적 `allow_polyset=True` 경로를 사용한다.

### Decision: task unit metadata는 호환상 `polysets` 시트에서 명시한다.

Reason: task unit canonical form과 binary encoder용 gloss를 member `canonical_form`/`gloss`에서 항상 안전하게 자동 추론하기는 어렵다. 따라서 `dict_ps_*.xlsx` 계열은 `polysets` 시트에 `ps_id`, `primary_e_id`, `member_e_ids`, `ps_canonical_form`, `disconti_allowed`, `ps_comp_id`, `gloss_intro`, `detect_ruleset_id`, `verify_ruleset_id`, `note`를 둔다. 시트명은 역사적으로 `polysets`이지만 실제 의미는 task-unit metadata이다. Encoder `text_b`는 `ps_canonical_form + "\n" + (gloss_intro + member item glosses)`로 만들며, `note`는 사람이 보는 관리 메모로만 보존한다.

### Decision: bundle export validation은 조용한 오작동보다 빠른 실패를 우선한다.

Reason: 300개 문법항목을 Excel로 관리하면 e_id/ps_id 오타, 잘못된 ruleset 연결, header 밀림 같은 문제가 반드시 생길 수 있다. 따라서 rule/component의 task unit은 `unit_id` → `ps_id` → `e_id` 순서로 해석하고, 새 dict에서는 `ps_id`를 기본으로 사용한다. `e_id`는 legacy 단일 item dict 호환을 위해 유지한다. `ps_id`는 `polysets.ps_id` 또는 `items.ps_id`에서 선언된 task unit이어야 하며, task unit이 참조한 detect/verify ruleset의 rule도 같은 task unit에 속해야 한다. stage가 맞지 않거나 regex compile이 실패하면 bundle export를 중단한다. warning은 pattern literal 의심처럼 사람이 확인할 수 있지만 runtime을 즉시 깨뜨리지는 않는 경우로 제한한다.
...
### Decision: ps_ce002 c1 `ㄴ/은/는데`에는 `nde` bridge를 사용한다.

Reason: ce002/ce003 polyset task `ps_ce002`는 `는데`, `은데`, `은 데`, `는 데`, `한데`처럼 표면 실현이 붙거나 띄어 쓰이거나 앞 음절 종성 ㄴ과 결합할 수 있다. 이전 프로젝트의 nde bridge도 Kiwi에 의존하지 않는 문자 기반 로직이므로, HanTalk에서는 `rule_components`의 `ps_id=ps_ce002`, `bridge_id=nde`로 연결하고 `src/detector/bridges.py`의 공용 matcher가 component span 후보만 반환하게 한다. TP/FP 의미 판단은 여전히 human review와 binary encoder 단계가 맡는다.
...
Reason: component span 조립에서 모든 순열을 허용하면 300개 문법항목 확장 단계에서 path 수가 폭증하고 응답속도가 흔들릴 수 있다. 따라서 `comp_order`를 기본 순서로 사용하고, `order_policy=fx`는 순서를 반드시 고정한다. `order_policy=fl`은 인접한 `fl` component끼리만 제한적으로 순서 뒤바뀜을 허용한다. anchor component는 `anchor_rank`가 비어 있지 않은 component 중 가장 작은 값을 가진 component로 보고 항상 기본 위치에 고정한다. `anchor_rank=0`이 가장 강한 anchor이며, 값이 커질수록 약한 anchor이다. 빈칸은 anchor 후보가 아니라는 뜻이다.
...
### Decision: 특정 component 주변 검증은 component-scoped verify target으로 처리한다.

Reason: df003에서 `char_window` 전체에 `적\s*(?:으로|인|일|에)` hard fail을 적용하자 후보 span과 무관한 주변 `자주적으로`, `직접적으로`, `긍정적인`이 잡혀 gold recall이 0.92로 떨어졌다. ps_ce002에서도 `가운데`를 제거하기 위해 `component_left_context=가`만 보면 `가는데` 같은 TP가 죽을 수 있다. 따라서 component 기반 후보에서는 문장 전체나 넓은 window가 아니라 선택된 component 자체와 바로 왼쪽/오른쪽 context만 검증한다. 공식 target은 `component_text`, `component_left_context`, `component_right_context`, `left_plus_component_text`이며, 모두 `component_id`를 필수로 요구한다. `component_left_context`, `component_right_context`, `left_plus_component_text`는 `detect_rules.context_chars`로 주변 non-space 문자 수를 rule별 조절할 수 있고 기본값은 1이다. N개보다 주변 문자가 적으면 가능한 만큼만 반환하며, component span을 찾지 못한 경우에만 recall 보호를 위해 verify rule을 skip한다.
...
### Decision: encoder examples에는 같은 문장 내 후보 순번 `instance_id`를 남긴다.

Reason: 한 문장 안에 같은 문법항목 후보가 두 번 이상 나올 수 있다. Search/review 단계에서는 `candidate_index`와 `hit_id`의 `cand01/cand02`로 구분되지만, encoder examples Excel/JSONL에서도 사람이 같은 문장 안의 후보를 추적할 수 있어야 한다. 따라서 `export_encoder_examples.py`는 같은 `item_id + text_id` 안의 후보를 span 시작 위치 순서로 `instance_id=1,2,...`로 배정한다. 정상 batch 자동화에서는 `text_id`를 안정적으로 부여해야 한다. 기존/수동 입력 호환을 위해 `text_id`가 없으면 `item_id + review_file + raw_text`로 fallback grouping을 하되 warning을 남기며, `--require-text-id`를 켜면 blank `text_id`를 오류로 처리한다. `merge_encoder_examples.py`는 기존 JSONL 호환을 위해 `instance_id`가 없으면 `1`로 간주한다.
...
### Decision: full-corpus search는 10,200행 shard 단위로 실행하고 advisory TP 150개 또는 FP/unclear 500개 상한에서 멈춘다.

Reason: ps_df004 `고 말`처럼 sampled batch에서 TP가 4개 수준인 항목은 일반 batch 반복만으로 positive 예문 100개 이상을 확보하기 어렵다. 그러나 전체 말뭉치를 한 번에 메모리에 올리면 속도와 안정성 위험이 커진다. 따라서 full-corpus search는 manifest의 top-level sampling 비율을 사용한 10,200행 shard를 순차 생성해 실행한다. 각 shard는 `search_corpus`, `prepare_codex_review`, `apply_first_pass_review`를 거친다. Advisory first-pass TP가 150개 이상이면 중단하되, TP 목표 전에 advisory FP가 500개 이상이거나 unclear가 500개 이상이면 사람 검수 부담이 너무 커진 것으로 보고 현재 merged first-pass 결과를 제출한다. 중간 실패 후 재시작할 때는 기존 shard report를 읽어 누적하고 해당 shard를 재실행하지 않는다. Advisory label은 Codex 1차 판단일 뿐이며, 최종 라벨은 human review가 확정한다.
...
### Decision: 다수 문법항목 review automation은 unit별 실패를 기록하고 다음 unit으로 계속 진행한다.

Reason: 30개 이상의 문법항목을 순차 처리할 때 한 unit이 FP/unclear 상한, max shard, 말뭉치 고갈, dict/bundle mismatch, gold recall 문제에 걸릴 수 있다. 이때 전체 batch 작업이 멈추면 긴 자동화의 효율이 크게 떨어진다. 따라서 `src/run_many_review_units.py`는 각 unit을 독립 실행 단위로 보고, 정상 stop_reason이면 제출본을 남긴 뒤 다음 unit으로 넘어간다. Blocked/failed 상태도 aggregate report에 기록하고 기본적으로 다음 unit으로 계속 진행한다. 단, 사용자가 원하면 `--stop-on-failure`로 첫 실패에서 멈출 수 있다. Relative path는 repo root/current working directory 기준으로 해석한다.
...
### Decision: encoder Excel에 수동 보강한 예문은 즉시 JSONL로 흡수한다.

Reason: `{item_id}_encoder_examples.xlsx`는 사람이 보기 좋은 gold-like 사본이지만 학습 SSOT는 아니다. 다만 사람이 supplemental negative를 Excel에 직접 추가하는 현실적 작업 흐름이 생길 수 있다. 이 경우 Excel을 계속 원본처럼 두지 않고 `src.import_encoder_excel_additions`로 Excel-only row를 item-level `{item_id}_encoder_pair_examples.jsonl`에 변환/흡수한 뒤 item Excel, summary, all aggregate를 다시 생성한다. 수동 보강분은 별도 audit report로 span_text 분포를 점검하여 detector-like negative인지 확인한다.
...
### Decision: item/polyset-specific dict/gold skeleton Excel을 반복 자동화 입력으로 허용한다.

Reason: 300개 문법항목을 한 번에 통합 Excel에 넣고 시작하면 초기 schema와 규칙 정제가 너무 무거워진다. 사용자는 새 unit마다 `dict_ps_??.xlsx`와 `gold_ps_??.xlsx` 같은 skeleton Excel을 제공할 수 있고, Codex/자동화는 working dict를 채운 뒤 bundle과 exported gold JSONL을 생성한다. 장기 aggregate `dict.xlsx`/`gold.xlsx`는 이후 검증된 내용을 흡수하는 관리 방식으로 둘 수 있다. 단, JSONL과 bundle은 항상 Excel에서 생성되는 산출물이며 사람이 직접 관리하는 원본이 아니다.

### Decision: 규칙 업데이트 시점은 gold FN과 human-confirmed systematic FP로 제한한다.

Reason: Codex/LLM 임시 판단만으로 detect/verify rule을 수정하면 학습 데이터와 runtime rule이 흔들릴 수 있다. 정규식 1차 수정은 gold 50 recall loop에서 FN을 없애기 위해 수행하고, corpus 단계의 규칙 수정은 사람이 확정한 labeled review에서 systematic FP가 확인될 때만 검토한다. 모든 규칙 수정 후에는 bundle을 재생성하고 gold recall=1을 다시 확인해야 하며, recall이 깨지는 수정은 채택하지 않는다.

### Decision: corpus rule refinement는 `FP/TP <= 2` 또는 `processed_batches >= 3`에서 멈춘다.

Reason: 일부 문법항목은 FP가 거의 없거나 반대로 특정 말뭉치에서 FP가 많이 생길 수 있다. 무한히 batch를 늘리거나 규칙을 과도하게 좁히면 자동화 비용이 커지고 gold recall 손상 위험이 커진다. 따라서 사람이 확정한 labeled review 기준으로 `FP/TP <= 2`이면 규칙 다듬기를 멈추고 결과를 제출한다. `FP/TP > 2`라도 `processed_batches >= max_processed_batches`이면 batch 추가와 규칙 다듬기를 멈추고 현재 확보량으로 encoder 필요성 또는 추가 전략을 판단한다. `summarize_review.py`는 이 판단을 `rule_refinement_status.should_consider_rule_update`와 `reason`으로 기록하되, 자동 수정용 `next_action`은 추가하지 않는다.
...
### Decision: corpus review batch 실행 전 dict/bundle sync mismatch는 차단한다.

Reason: dict Excel은 사람이 관리하는 SSOT이고 bundle은 자동 생성 산출물이므로, first-pass나 corpus search 단계가 dict를 자동 수정해서는 안 된다. 대신 `run_corpus_review_batch.py`에 `--dict`가 제공되면 dict를 메모리에서 다시 bundle로 export하고 현재 `--bundle`의 해당 unit slice와 비교한다. 비교 대상에는 runtime unit, 해당 unit의 rule_components, detect/verify rule이 포함된다. `rule_components.bridge_id`, `comp_surf`, gap/order 값은 span 조립과 component-scoped verify 동작에 직접 영향을 주므로 제외하지 않는다. 비교는 export된 bundle끼리 수행하므로 bridge raw pattern과 expanded pattern 차이로 인한 false diff를 피한다. `component_id`의 blank와 null은 동일하게 정규화한다. 불일치하면 `status=blocked`, `failure_reason=dict_bundle_mismatch`로 search를 중단하고 sync report를 남긴다. 정상 운영에서는 이 검사를 기본으로 두고, `--skip-dict-bundle-sync`는 의도적 진단 상황에서만 사용한다.
...
### Decision: unit별 first-pass FP 주의 유형은 작은 repository로 관리한다.

Reason: Codex/LLM의 1차 TP/FP 판단에서 사람이 알려준 주의점을 모델 기억에만 의존하면 다음 세션이나 다른 unit에서 반복 실수가 생긴다. 따라서 `configs/first_pass_cautions/{unit_id}.json`을 두고, `apply_first_pass_review.py`가 first-pass profile 적용 전에 이 repository를 먼저 읽어 advisory label/reason을 채운다. 이 repository는 자동 정답 판정기가 아니라 “검토 전 reminder”이며, 최종 라벨은 여전히 사람이 쓰는 `human_label`/`span_status`이다. `ps_ce002`의 첫 caution은 `ㄴ/은/는 + 의존명사 데`가 띄어쓰기 없이 붙어 `ㄴ/은/는데` 연결어미처럼 보이는 STT/말뭉치 혼동 유형이다.
...
### Decision: 여러 labeled review finalize/export/merge는 multi-unit wrapper로 실행한다.

Reason: 여러 문법항목의 labeled review가 동시에 준비되면 unit별 `finalize_labeled_review.py` 실행과 전체 `merge_encoder_examples.py` 재생성을 수동으로 반복하기 쉽고, 일부 unit 실패 때문에 전체 처리가 멈추거나 성공한 item-level JSONL merge를 빠뜨릴 위험이 있다. 따라서 `src/finalize_many_labeled_reviews.py`를 두어 manifest 기준으로 unit별 finalize/export를 순차 실행하고, 성공한 item-level encoder JSONL만 모아 `all_encoder_*` aggregate를 자동 재생성한다. 성공 export가 0개이면 merge를 실패시키지 않고 `skipped_no_successful_exports`로 기록한다. `--only-unit` 또는 `--start-at-unit`으로 일부 unit만 실행한 경우 report에 `includes_only_this_run_units=true`를 남겨 산출 aggregate가 전체 데이터가 아닐 수 있음을 명시한다.
