...
| Phase | 범위 | 도구 | 목표 |
| --- | --- | --- | --- |
| Phase 1 | df003 완료 + ps_ce002 확장 + all-ps_id task-unit 전환 | Codex + Python CLI + CSV/Excel | task-unit 자동화 경로 검증 |
...
| current task name | ps_id task-unit automation |
| current goal | 모든 문법항목에서 `ps_id`를 detect_unit_id/encoder_task_id로 사용하고, dict/gold skeleton에서 bundle, gold recall, corpus review, encoder export까지 반복 자동화하기 |
...
Phase 1 pilot부터 아래 반자동 루프를 기준으로 합니다. df003처럼 단일 teaching item도 `ps_df003` task unit으로, ps_ce002처럼 여러 teaching item을 묶는 경우도 `ps_ce002` task unit으로 같은 루프를 사용합니다.
...
First-pass 전에 unit별 FP 주의 유형 repository를 먼저 적용할 수 있습니다. 기본 위치는 `configs/first_pass_cautions/{unit_id}.json`입니다. 이 repository는 Codex가 이전 human review에서 배운 “주의해서 FP로 볼 유형”을 코드가 매번 읽도록 만든 reminder입니다. 예를 들어 `ps_ce002`에는 `ㄴ/은/는 + 의존명사 데`가 띄어쓰기 없이 붙어 `ㄴ/은/는데` 연결어미처럼 보이는 STT/말뭉치 혼동 유형을 `caution_dep_noun_de_spacing`으로 기록합니다. Caution은 first-pass profile보다 먼저 적용되지만 여전히 참고 판단일 뿐이며, 최종 라벨은 사람이 작성하는 `human_label`과 `span_status`입니다. Report에는 caution source, hit count, examples가 별도로 기록됩니다.
...
단, processed labeled review 기준 TP 수가 `min_tp_for_batch_mode` 미만이면 일반 sampled batch 반복으로는 positive 예문 확보 효율이 낮다고 보고 `next_action=switch_to_full_corpus_search`를 기록합니다. 기본값은 `30`입니다. 이 상태에서는 `finalize_labeled_review.py`가 encoder export를 진행하지 않고 전체 말뭉치 offline search를 권장합니다.

여러 unit의 labeled review가 한꺼번에 준비되면 `src/finalize_many_labeled_reviews.py`를 사용합니다. 이 orchestrator는 manifest에 적힌 unit을 순서대로 `finalize_labeled_review.py`에 위임하고, 성공적으로 생성된 item-level `{unit_id}_encoder_pair_examples.jsonl`만 모아 마지막에 `merge_encoder_examples.py`로 `all_encoder_*` aggregate를 재생성합니다. 한 unit이 cleanup, full-corpus 전환, 파일 누락 등으로 blocked/failed가 되어도 기본적으로 다음 unit으로 계속 진행하며, `--stop-on-failure`가 있을 때만 중단합니다. 성공 export가 0개이면 merge는 실패로 터뜨리지 않고 `skipped_no_successful_exports`로 기록합니다. `--only-unit` 또는 `--start-at-unit`으로 일부 unit만 처리한 경우 report의 merge 섹션에 `includes_only_this_run_units=true`를 남겨 해당 `all_encoder_*`가 전체 aggregate가 아닐 수 있음을 명시합니다.

전체 말뭉치 offline search는 `src/run_full_corpus_review.py`를 사용합니다. 이 runner는 전체 말뭉치를 하나의 거대한 JSONL로 만들지 않고, manifest의 top-level sampling 비율인 10,200행 shard를 순차 생성해 검색합니다. shard 구성은 일상대화 5,000행, 뉴스 700행, 비출판물 2,000행, 학습자 말뭉치 2,500행입니다. 각 shard는 gold gate와 dict/bundle sync를 통과한 bundle로 검색하고, `prepare_codex_review.py`와 `apply_first_pass_review.py`를 거쳐 first-pass review 파일을 만듭니다. 기본 중단 기준은 advisory first-pass TP 150개, advisory FP 500개, advisory unclear 500개, 또는 `max_shards=50`입니다. FP/unclear 상한은 사람 검수 부담을 막기 위한 안전장치이며 TP 목표보다 먼저 도달하면 현재까지 찾은 merged first-pass 결과를 제출합니다. 이미 완료된 shard report가 있으면 schema와 `domain_state_after`를 검증한 뒤 report를 읽어 누적하므로, 긴 실행이 중간에 끊겨도 idempotent하게 재시작할 수 있습니다.

긴 full-corpus 실행 중 진행 상황은 `{unit_id}_full_corpus_progress.json`과 `{unit_id}_full_corpus_progress.jsonl`에 계속 기록합니다. 최신 상태를 빠르게 볼 때는 `.json`을 읽고, shard별 진행 이력을 복기할 때는 `.jsonl`을 봅니다. Runner 종료 전에는 merged first-pass CSV/XLSX가 실제로 존재하고 비어 있지 않은지 검증하며, 결과를 `final_output_validation`에 기록합니다. 최종 산출물 누락은 조용히 통과시키지 않고 오류로 처리합니다.

Full-corpus shard에서 특정 domain이 고갈되면 이후 shard에서는 그 domain을 건너뛰고 부족분을 `news` domain에 backfill합니다. Cursor는 요청 수가 아니라 실제 선택된 row 수(`selected_count`)만큼만 전진합니다. `selected < requested`이면 단기 운영에서는 해당 domain을 exhausted로 표시하고 `exhausted_reason=selected_below_requested`를 report에 남깁니다. Backfill chain은 만들지 않습니다. 즉 `news`까지 고갈되면 더 이상 다른 domain으로 재분배하지 않고 `backfill_domain_exhausted_before_target` 또는 `all_corpora_exhausted_before_target`으로 정상 종료한 뒤, 현재까지 찾은 merged first-pass 결과를 사람이 검수합니다.

Full-corpus first-pass TP는 `codex_review_label=tp` 기준의 advisory count이며 최종 학습 label이 아닙니다. Runner가 만드는 merged first-pass CSV/XLSX를 사람이 검수해 `human_label`과 `span_status`를 확정한 뒤, 다시 `finalize_labeled_review.py`를 실행합니다.

여러 문법항목을 순차 처리할 때는 `src/run_many_review_units.py`를 사용합니다. 이 orchestrator는 manifest에 적힌 unit을 순서대로 실행하고, 각 unit의 실제 처리는 `run_corpus_review_batch.py` 또는 `run_full_corpus_review.py`에 위임합니다. 한 unit이 `target_first_pass_tp_reached`, `first_pass_fp_cap_reached`, `first_pass_unclear_cap_reached`, `max_shards_reached`, `backfill_domain_exhausted_before_target`, `all_corpora_exhausted_before_target` 등 정상 stop_reason으로 끝나면 해당 unit의 제출본과 report를 남기고 다음 unit으로 계속 진행합니다. `gold_recall_fix_required`, `dict_bundle_mismatch`, 파일 누락 같은 blocked/failed 상태도 aggregate report에 기록하고 기본적으로 다음 unit으로 넘어갑니다. 단, `--stop-on-failure`를 주면 첫 blocked/failed unit에서 중단합니다. `run_many_review_units.py` manifest의 상대 경로는 repo root/current working directory 기준으로 해석합니다.

예외적으로 사람이 `{item_id}_encoder_examples.xlsx`에 supplemental 예문을 직접 추가한 경우, 이 Excel을 장기 SSOT로 삼지 않습니다. 즉시 `src.import_encoder_excel_additions`로 Excel-only row를 `{item_id}_encoder_pair_examples.jsonl`에 흡수한 뒤 item-level Excel/summary와 `all_encoder_*` aggregate를 다시 생성합니다. 이 경로는 수동 보강을 JSONL SSOT로 복구하기 위한 recovery/bridge 절차이며, 전체 `all_encoder_*` 파일은 여전히 직접 수정하지 않습니다.
...
- TP가 `min_tp_for_batch_mode` 미만이면 sampled batch 반복 대신 전체 말뭉치 offline search로 전환합니다.
- 전체 말뭉치 offline search는 10,200행 shard 단위로 실행하고, 기본적으로 advisory first-pass TP가 150개 이상이면 중단합니다.
- advisory first-pass FP가 500개 이상이거나 unclear가 500개 이상이면 TP 목표 전이라도 중단하고 현재 merged first-pass 결과를 사람이 검수합니다.
...
| `configs/detector/detector_bundle_ps_??.json` | task-unit skeleton dict에서 생성한 개발/검증용 detector bundle | 자동화 | DetectorEngine, gold test |
...
| `datasets/gold/gold_ps_??.xlsx` | task-unit 자동화 시작용 정규식 gold skeleton | 사람 | `export_gold.py` |
| `datasets/dict/dict.xlsx` | 장기 aggregate dict Excel | 사람 | `export_bundle.py` |
| `datasets/dict/dict_ps_??.xlsx` | task-unit 자동화 시작용 dict skeleton 또는 working dict | 사람 + Codex | `export_bundle.py` |
| `configs/first_pass_cautions/{unit_id}.json` | Codex 1차 판단 전에 적용할 unit별 FP 주의 유형 repository. 이전 human review에서 얻은 주의점을 코드가 읽는 reminder로 사용 | 사람 + Codex | `apply_first_pass_review.py` |
| `exported_gold/{unit_id}_gold_50.jsonl` | gold Excel에서 자동 생성한 unit별 정규식 gold positive 50개 | 자동화 | gold test CLI |
| `HanTalk_arti/example_making/{item_id}/{item_id}_manual_supplement_audit.json` | Excel 수동 보강분의 span_text 분포와 기계적 품질 점검 report | 자동화 | 사람 + Codex |
...
사람이 관리하는 정규식 gold 원본은 Excel입니다. 장기적으로는 `datasets/gold/gold.xlsx` 같은 aggregate 파일로 모을 수 있지만, 현재 반복 자동화에서는 사용자가 `datasets/gold/gold_ps_??.xlsx` 같은 task-unit-specific skeleton Excel을 제공할 수 있습니다.
...
- 장기 aggregate `gold.xlsx`를 만들 때도 task-unit-specific skeleton에서 검증된 내용을 흡수하는 방식으로 관리합니다.

## dict Excel 설계 원칙

사람이 관리하는 문법항목 사전 원본은 Excel입니다. 장기적으로는 통합 `dict.xlsx`를 둘 수 있지만, 현재 반복 자동화에서는 `datasets/dict/dict_ps_??.xlsx` 같은 task-unit-specific skeleton 또는 working dict를 사용할 수 있습니다. `configs/grammar_items.yaml`은 필요하면 dict Excel에서 생성되는 기계 친화적 산출물로 둡니다.

dict Excel의 기본 시트는 아래를 사용합니다. `polysets`라는 시트 이름은 호환을 위해 유지하지만, 현재 의미는 “실제 polyset 전용”이 아니라 `ps_id` 기반 detect/encoder task unit metadata입니다.

```text
items
rule_components
detect_rules
polysets   # ps_id 기반 task unit metadata. 단일 item wrapper와 실제 multi-member polyset 모두 포함 가능
```
...
사람이 관리하는 문법항목 사전의 SSOT는 dict Excel입니다. 단, runtime detector는 Excel을 직접 읽지 않습니다. 통합 `dict.xlsx` 또는 task-unit-specific `dict_ps_??.xlsx`는 아래 명령으로 runtime bundle로 export합니다.
...
개별 task unit 개발 중에는 main bundle을 덮어쓰지 않기 위해 별도 bundle을 생성합니다.
...
- 단일-member `ps_id` task unit은 일반 item runtime unit처럼 실행합니다. 예: `ps_df003`.
- multi-member polyset `ps_id` task unit은 기본적으로 자동 실행하지 않으며, `active_unit_ids`에 ps_id를 명시하고 `allow_polyset=True`를 넘긴 경우에만 실행합니다. 예: `ps_ce002`.
- 기존 실험용 옵션 `allow_experimental_polyset=True`는 호환용으로 남기되, multi-member ps_id의 공식 경로는 `allow_polyset=True`입니다.
...
## 전체 문법항목 task-unit 2-ID 체계

모든 문법항목은 아래 2-ID 체계로 운영합니다. `e_id`는 사람이 가르치는 teaching item ID이고, `ps_id`는 정규식 detect와 binary encoder 오탐 필터링을 수행하는 task unit ID입니다. 단일 문법항목도 `ps_id`를 가질 수 있습니다.

| 층위 | ID | 역할 |
| --- | --- | --- |
| teaching item | `e_id` (`df003`, `ce002`, `ce003`) | 사람이 가르치는 문법항목 의미와 gloss를 보존 |
| task unit | `ps_id` (`ps_df003`, `ps_ce002`) | 정규식 detect, corpus search, human review, binary encoder 오탐 필터링 단위 |

원칙:

- HanTalk의 현재 사용자 발화 기본 판정은 다의의미 분별이 아니라 오탐 제거입니다.
- `ps_id`는 detect_unit_id이자 encoder_task_id입니다.
- `ps_id`는 이름상 polyset ID이지만, 현재 코드와 문서에서는 “실제 polyset만”을 뜻하지 않습니다. 단일 item wrapper와 multi-member polyset을 모두 포함하는 task unit ID입니다.
...
- binary encoder의 `text_b`는 task unit canonical form과 task unit encoder gloss를 사용합니다.

`polysets` 시트의 최소 schema:
...
```text
items.e_id = df003
items.ps_id = ps_df003
polysets.ps_id = ps_df003
polysets.member_e_ids = df003
runtime_units["ps_df003"].unit_type = item
```

multi-member task unit 예:

```text
items.e_id = ce002; ce003
items.ps_id = ps_ce002
polysets.ps_id = ps_ce002
polysets.member_e_ids = ce002;ce003
runtime_units["ps_ce002"].unit_type = polyset
```

encoder `text_b`는 아래 방식으로 생성합니다.
...
- `origin_e_id`는 component lookup 기준입니다. ps_id task-unit 확장 시 `unit_id`와 teaching `e_id`가 달라질 수 있으므로 지금부터 보존합니다.
...
- 새 dict에서 `ps_id` 기반 rule/component는 `polysets.ps_id`에 존재해야 합니다. legacy/transition 호환을 위해 exporter는 `items.ps_id`만 있는 task unit도 읽을 수 있지만, 장기 SSOT에서는 `polysets` 시트에 task-unit metadata를 명시합니다.
- task unit이 참조한 `detect_ruleset_id`와 `verify_ruleset_id` 안의 rule은 같은 task unit에 속해야 합니다.
...
- pattern이 `r"..."` 같은 Python literal처럼 보이는 경우, `ps_id` 또는 legacy `polyset_id`가 없는 경우, verify rule인데 hard_fail=false인 경우는 warning입니다.
...
component bridge 공용화와 df003 component span 조립도 Phase 1에서 구현되었습니다. ce002/ce003부터는 `ps_id` 기반 multi-member task unit의 gold recall 평가 경로도 열었습니다. 아래 항목들은 아직 당장 구현하지 않고, 다음 단계에서 실행 가능성과 schema 안정성을 검토합니다.
...
`configs/grammar_items.yaml`은 Phase 1 초기에 만든 pilot 보조 config입니다. 현재 장기 SSOT는 사람이 관리하는 dict/gold Excel입니다. 통합 `dict.xlsx`/`gold.xlsx`와 task-unit-specific `dict_ps_??.xlsx`/`gold_ps_??.xlsx`는 모두 Excel SSOT 계열이며, JSONL과 bundle은 자동 생성 산출물입니다.
...
`text_b`는 dict Excel을 직접 읽지 않고 detector bundle에서 생성합니다. 기본적으로 encoder export의 `item_id`는 detect/encoder task unit id이며, `runtime_units[ps_id]`의 `canonical_form`/`gloss`를 사용합니다. legacy 단일 item dict만 `items_by_e_id[e_id]` 또는 `runtime_units[e_id]` fallback을 허용합니다.

```json
{"schema_version":"hantalk_encoder_pair_example_v1","input_construction_version":"hantalk_binary_pair_v1","item_id":"df003","example_id":"df003-pos-0001","instance_id":1,"label":1,"label_name":"positive","example_role":"pos_disconti","negative_type":null,"split":"train","text_a":"저는 제주도에 가 [SPAN]본 적[/SPAN]이 [SPAN]있[/SPAN]어요.","text_b":"ㄴ/은 적 있/없\n어떤 행위나 상태를 해 본 경험이 있거나 없음을 나타냄.","raw_text":"저는 제주도에 가 본 적이 있어요.","span_segments":[[10,13],[15,16]],"span_key":"10:13|15:16","span_text":"본 적 ... 있","canonical_form":"ㄴ/은 적 있/없","gloss":"어떤 행위나 상태를 해 본 경험이 있거나 없음을 나타냄.","pattern_type":"disconti","source_hit_id":"daily_conversation_b000_000001-cand01","corpus_domain":"daily_conversation","detect_rule_ids":["r_df003_d01"]}
```

`instance_id`는 같은 `item_id + text_id` 안의 여러 후보를 span 시작 위치 순서로 1부터 매긴 값입니다. 예를 들어 한 문장에 `ㄴ/은/는데` 후보가 두 번 있으면 첫 후보는 `instance_id=1`, 두 번째 후보는 `instance_id=2`로 저장합니다. 이는 사람이 Excel에서 같은 문장 안의 여러 instance를 구분하고, 학습/오류 분석에서 한 문장 다중 후보를 추적하기 위한 보조 ID입니다.

정상 batch 자동화에서는 `text_id`를 반드시 안정적으로 부여해야 합니다. 기존/수동 입력 호환을 위해 `text_id`가 없는 labeled row도 export는 가능하지만, 이 경우 instance grouping은 `item_id + review_file + raw_text` fallback을 사용하고 summary warning을 남깁니다. 일반 자동화 품질 점검에서는 `export_encoder_examples.py` 또는 `finalize_labeled_review.py`에 `--require-text-id`를 붙여 blank `text_id`를 오류로 처리할 수 있습니다.
...
- item별/전체 encoder Excel ledger는 `instance_id`를 포함합니다. 전체 merge에서는 기존 파일 호환을 위해 `instance_id`가 없는 오래된 JSONL row는 `1`로 간주합니다.
...
Phase 1에서 아래 명령 계열이 동작하는 것을 목표로 합니다. legacy df003 단일 item 경로는 이미 검증했고, 앞으로는 df003도 `ps_df003` 같은 단일-member ps_id task unit으로 운영합니다. `ps_ce002`처럼 multi-member task unit은 `--allow-polyset`과 별도 bundle을 사용해 검증합니다.
