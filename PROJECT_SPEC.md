...
전체 말뭉치 offline search는 `src/run_full_corpus_review.py`를 사용합니다. 이 runner는 전체 말뭉치를 하나의 거대한 JSONL로 만들지 않고, manifest의 top-level sampling 비율인 10,200행 shard를 순차 생성해 검색합니다. shard 구성은 일상대화 5,000행, 뉴스 700행, 비출판물 2,000행, 학습자 말뭉치 2,500행입니다. 각 shard는 gold gate와 dict/bundle sync를 통과한 bundle로 검색하고, `prepare_codex_review.py`와 `apply_first_pass_review.py`를 거쳐 first-pass review 파일을 만듭니다. 기본 중단 기준은 advisory first-pass TP 150개 또는 `max_shards=50`입니다. 이미 완료된 shard report가 있으면 schema와 `domain_state_after`를 검증한 뒤 report를 읽어 누적하므로, 긴 실행이 중간에 끊겨도 idempotent하게 재시작할 수 있습니다.
